...
{ 
	$defaults = array(
		...
		'prev_link_class' => 'nav-prev alignright shadow',
		'next_link_class' => 'nav-next alignleft shadow',
		'prev_text' => __('Next', 'ci_theme'),
		'next_text' => __('Previous', 'ci_theme'),
	...
	if ($wp_query->max_num_pages > 1): ?>
		...
		>
			...
			<?php else: ?>
				<div <?php echo (empty($args['next_link_class']) ? '' : 'class="'.$args['next_link_class'].'"'); ?>><?php previous_posts_link( '<span class="nav-next-symbol nav-symbol">&laquo;</span>' . $args['next_text']); ?></div>
				<div <?php echo (empty($args['prev_link_class']) ? '' : 'class="'.$args['prev_link_class'].'"'); ?>><?php next_posts_link( $args['prev_text'] . '<span class="nav-next-symbol nav-symbol">&raquo;</span>' ); ?></div>
			<?php endif; ?>
		</div>
	<?php endif;
}
